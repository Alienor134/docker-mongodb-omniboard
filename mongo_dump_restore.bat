...
REM Restore the dump to local MongoDB (if running)
echo.
echo Attempting to restore to local MongoDB...
mongorestore --nsInclude=%DB_NAME%.fs.* --db=%DB_NAME% dump-folder/%DB_NAME%

if %errorlevel% equ 0 (
    echo  Restore to local %DB_NAME% completed
) else (
    echo  Local restore failed (local MongoDB may not be running)
    echo The dump is available in dump-folder/%DB_NAME%/
)
